				  ...
				  const char **mac)
{
	...
	if (device->data) {
		...
		//AG plat->fix_mac_speed = data->fix_mac_speed;
		plat->bus_setup = data->bus_setup;
		plat->setup = data->setup;
		plat->free = data->free;
		//AG plat->init = data->init;
		plat->exit = data->exit;
	}
	...
	*/
	 ...
	 */
	//AG
	if (of_property_read_u32(np, "phy-addr", &phyaddr) == 0) 
	{
		if ((-1 == phyaddr) || ((phyaddr >= 0) && (phyaddr < PHY_MAX_ADDR))) 
		{
	...
	else if (of_property_read_u32(np, "snps,phy-addr", &plat->phy_addr) == 0)
 ...
 */
extern int stmmac_plat_init(struct platform_device *pdev, void* private);
