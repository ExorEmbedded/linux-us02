...
static void ultievc_setclkctrl(void)
{
    ...
    u32 clk_sel = 0;
    u32 clk_div = 0;
    u32 desired_freq = displayconfig[displayid].pclk_freq;
    u32 clkctrl = 0;
    ...
    if(displayconfig[displayid].pclk_inv)
        clkctrl |= (1 << 7);
...
int ultievc_setvideomode(void)
{
        ...
        printk(KERN_INFO "*** Resolution....: %ld x %ld\n",displayconfig[displayid].rezx,displayconfig[displayid].rezy);
        printk(KERN_INFO "*** Pclk..........: Freq=%ld Inv=%ld\n",displayconfig[displayid].pclk_freq,displayconfig[displayid].pclk_inv);
        printk(KERN_INFO "*** Bpp...........: %ld\n",displayconfig[displayid].bpp);
        printk(KERN_INFO "*** Hsync.........: FP=%ld BP=%ld W=%ld Inv=%ld\n",displayconfig[displayid].hs_fp,displayconfig[displayid].hs_bp,displayconfig[displayid].hs_w,displayconfig[displayid].hs_inv);
        printk(KERN_INFO "*** Vsync.........: FP=%ld BP=%ld W=%ld Inv=%ld\n",displayconfig[displayid].vs_fp,displayconfig[displayid].vs_bp,displayconfig[displayid].vs_w,displayconfig[displayid].vs_inv);
        printk(KERN_INFO "*** Blank.........: Inv=%ld\n",displayconfig[displayid].blank_inv);
        
        /* Horizontal resolution and Hsync */
        writel(displayconfig[displayid].hs_fp, evcbase + HSY_FP);
        writel(displayconfig[displayid].hs_bp, evcbase + HSY_BP);
        writel(displayconfig[displayid].hs_w, evcbase + HSY_W);
        writel(displayconfig[displayid].rezx, evcbase + HRES);
        
        /* Vertical resolution and Vsync */
        writel(displayconfig[displayid].vs_fp, evcbase + VSY_FP);
        writel(displayconfig[displayid].vs_bp, evcbase + VSY_BP);
        writel(displayconfig[displayid].vs_w, evcbase + VSY_W);
        writel(displayconfig[displayid].rezy, evcbase + VRES);
        
        /* Sync signals polarity */
        sctrl = SCTRL_VSEN | SCTRL_HSEN | SCTRL_ENEN;
        
        if(!displayconfig[displayid].hs_inv)
            sctrl |= SCTRL_HSPOL;
        
        if(!displayconfig[displayid].vs_inv)
            sctrl |= SCTRL_VSPOL;

        if(!displayconfig[displayid].blank_inv)
            sctrl |= SCTRL_ENPOL;
        ...
        if(displayconfig[displayid].bpp == 16)
        ...
        writel(2 * displayconfig[displayid].rezx - 1, evcbase + LSTRIPE);
