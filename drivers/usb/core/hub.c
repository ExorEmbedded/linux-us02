...
int usb_hub_clear_tt_buffer_irq(struct urb *urb)
{
	struct usb_device	*udev = urb->dev;
	int			pipe = urb->pipe;
	struct usb_tt		*tt = udev->tt;
	...
	//spin_lock_irqsave (&tt->lock, flags);
	list_add_tail (&clear->clear_list, &tt->clear_list);
	//do not call the schedule work if the function is called by a IRQF_NO_THREAD handler
	//schedule_work(&tt->clear_work);
	//spin_unlock_irqrestore (&tt->lock, flags);
	return 0;
}
EXPORT_SYMBOL_GPL(usb_hub_clear_tt_buffer_irq);
